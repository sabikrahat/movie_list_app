...
class _TextScrollState extends State<TextScroll> {
  ...
  Future<void> _animateEndless() async {
    ...
    if (!_available) return;
    await _scrollController.animateTo(singleRoundExtent, duration: duration, curve: Curves.linear);
    if (!_available) return;
  ...
  Future<void> _animateBouncing() async {
    ...
    if (!_available) return;
    await _scrollController.animateTo(minExtent, duration: duration, curve: Curves.linear);
    if (!_available) return;
    if (widget.pauseBetween != null) {
